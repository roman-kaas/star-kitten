...
export class Repository<T> {
  ...
  constructor(
    ...
    this.tableRegistry = db.registry.getTableRegistry(this.db.registry.getTableForClass(name));
    this.init();
  }

  init = () => {
    if (!this.tableRegistry.dbTable) {
      console.warn(`attempting to init invalid tableRegistry: ${JSON.stringify(this.tableRegistry)}`);
      return;
    }
  ...
  public save = (entity: T) => {
    ...
    console.log(this.tableRegistry.dbColumns);
  ...
  private hydrate = (row: T, parent: any = null) => {
    ...
    for (const key of Object.keys(this.tableRegistry.computedKeys)) {
      ...
      if (colDef.type === 'one-to-many') {
        ...
        const relationRegistry = this.db.registry.getTableRegistry(this.db.registry.getTableForClass(oneToMany.relationClassname));
