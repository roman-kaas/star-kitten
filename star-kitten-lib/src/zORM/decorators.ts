 ...
 * @param {ColumnDecoratorOptions} Options
 */
export function ManyToOne<T>(referencedTable: string, fn: PropertyGetter<T>, options?: ManyToOneDecoratorOptions) {
...
export function OneToOne<T>(referencedTable: string, fn: PropertyGetter<T>, options?: ColumnDecoratorOptions) {
  return (target: any, key: string) => {
    const metadata = {
      key,
      classname: target.constructor.name,
 ...
 * @param {PropertyGetter<T>} fn -- function to get the property to reference
 */
export function OneToMany<T>(referencedTable: string, fn: PropertyGetter<T>) {
  return (target: any, key: string) => {
    const metadata = {
      key,
      classname: target.constructor.name,
...
export function JoinOne<T>(referencedTable: string, fn: PropertyGetter<T> | MultiPropertyGetter<T>) {
  return (target: any, key: string) => {
    const metadata = {
      key,
      classname: target.constructor.name,
...
export function JoinMany<T>(referencedTable: string, fn: PropertyGetter<T> | MultiPropertyGetter<T>) {
  return (target: any, key: string) => {
    const metadata = {
      key,
      classname: target.constructor.name,
