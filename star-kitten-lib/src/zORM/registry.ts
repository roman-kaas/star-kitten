...
export class Registry {
  ...
  private classTableMap: { [className: string]: string } = {};
  ...
  private buildTable(metadata: HasTableMetaData) {
    const dbTable = metadata.options?.name || metadata.classname.toLowerCase()
    const tr = this.getTableRegistry(dbTable);
    this.classTableMap[metadata.classname] = dbTable;
  ...
  private buildColumn(metadata: HasColumnMetaData) {
    const tr = this.getTableRegistry(this.classTableMap[metadata.classname]);
  ...
  private buildOneToMany(metadata: HasOneToManyMetaData) {
    const tr = this.getTableRegistry(this.classTableMap[metadata.classname]);
    const referenceTable = this.getTableRegistry(metadata.options.referencedTable);
    ...
    const otm: OneToManyMetaData = {
      ...
      relationClassname: metadata.options.referencedTable,
  ...
  private buildOneToOne(hasdata: HasOneToOneMetaData) {
    const tr = this.getTableRegistry(this.classTableMap[hasdata.classname]);
    const referenceTable = this.getTableRegistry(hasdata.options.referencedTable);
    ...
    const data: OneToOneMetaData = {
      ...
      designType: designType.name,
      relationClassname: hasdata.options.referencedTable,
  ...
  getTableForClass(classname: string) {
    return this.classTableMap[classname];
  }
